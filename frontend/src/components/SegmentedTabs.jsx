...
//
// Visual spec (locked theme — blue #4FA3CE, neutrals, rounded pills):
//   container: #eef1f4 pill, radius 14, padding 5, flex with 6px gap
...
export default function SegmentedTabs({ segments, value, onChange }) {
  return (
    <div style={{ display: 'flex', gap: '6px', background: '#eef1f4', borderRadius: '14px', padding: '5px' }}>
      {segments.map(seg => {
        const active = value === seg.id;
        const textColor = seg.color || (active ? '#1d1d1f' : '#7a7a7a');
        return (
          <button
            key={seg.id}
            type="button"
