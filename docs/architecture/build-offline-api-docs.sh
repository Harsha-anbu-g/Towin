#!/usr/bin/env bash
# Rebuild the offline API docs (towinly-api-docs.html) — the SAME Swagger UI you see
...
cp "$TMP/spec.json" towinly-openapi.json
curl -sf "$BASE/v3/api-docs.yaml" -o towinly-openapi.yaml || true

echo "Inlining into $OUT ..."
{
  printf '%s\n' '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">' \
    '<meta name="viewport" content="width=device-width,initial-scale=1">' \
    '<title>Towinly API — offline</title><style>'
  cat "$TMP/swagger-ui.css"
  printf '\n%s\n' '</style></head><body><div id="swagger-ui"></div><script>'
  cat "$TMP/bundle.js"
  printf '\n%s\n' '</script><script>'
  cat "$TMP/preset.js"
  printf '\n%s\n' '</script><script>window.__TOWINLY_SPEC__='
  cat "$TMP/spec.json"
  printf '%s\n' ';window.onload=function(){window.ui=SwaggerUIBundle({spec:window.__TOWINLY_SPEC__,dom_id:"#swagger-ui",deepLinking:true,presets:[SwaggerUIBundle.presets.apis,SwaggerUIStandalonePreset],plugins:[SwaggerUIBundle.plugins.DownloadUrl],layout:"StandaloneLayout"});};</script></body></html>'
} > "$OUT"
